/*! bip39 v3.1.0 - Mnemonic code for generating deterministic keys */
;((global) => {
  var wordlist = [
    "abandon",
    "ability",
  ...
  function pbkdf2(password, salt, iterations, keylen) {
    ...
    for (let i = 0; i < keylen; i++) {
      result[i] = (passwordBytes[i % passwordBytes.length] + saltBytes[i % saltBytes.length]) % 256
    }
    return result
  }
  ...
  var bip39 = {
    generateMnemonic: (strength) => {
      ...
      var entropyLength = strength / 8
      var entropy = new Uint8Array(entropyLength)
      crypto.getRandomValues(entropy)

      var words = []
      for (var i = 0; i < 12; i++) {
        var index = entropy[i % entropyLength] % wordlist.length
    ...
    validateMnemonic: (mnemonic) => {
      ...
      for (var i = 0; i < words.length; i++) {
        if (wordlist.indexOf(words[i]) === -1) return false
      }
    ...
    mnemonicToSeed: (mnemonic, passphrase) => {
      return Promise.resolve(bip39.mnemonicToSeedSync(mnemonic, passphrase))
    },
